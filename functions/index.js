...
const decode64 = function(input) {
    ...
    if (base64test.exec(input)) {
        console.error("There were invalid base64 chars in the input text.\n" +
    ...
    .onCreate((change, context) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            chatRef.onSnapshot((snapChat) => {
                ...
                msgRef.onSnapshot((snapMessage) => {

                    const msgDoc = snapMessage.data();

                    console.log('[MESSAGE]', msgDoc);
                    console.log('[USERS]', Object.keys(chatDoc.users));
                    console.log('[FROM BASE64]', encode64(msgDoc.from));

                    const messageFrom = msgDoc.from;
                    const messageTo = Object.keys(chatDoc.users)
                    .filter((userId) => userId !== encode64(msgDoc.from))[0];

                    console.log('[FROM]', messageFrom);
                    console.log('[TO]', decode64(messageTo));
                    console.log('[LAST MESSAGE]', msgDoc.content);
                    console.log('[LAST MESSAGE TIME]', new Date());

                    db.collection('users').doc(decode64(messageTo))
                        .collection('contacts').doc(messageFrom).set({
                            lastMessage: msgDoc.content,
